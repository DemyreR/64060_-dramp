...
```
The best method is "Ward" with the ac value of 0.8853988.
<br>
...
After looking at the Dendrogram's height differences and using the Elbow Method
(WSS), I chose 3 clusters.
<br>
...
```
After comparing the cluster assignments and the HC data, I found that the 
assignments are 95% accurate. Meaning the structure of the clusters are
stable, but while testing different seeds there seems to be an unbalance 
depending on the partition. Further validation may be recommended.
<br>
...
The first cluster from the original clustering is perfect for the set of
cereals requested by the elementary public schools. This cluster of cereals
is very healthy and there are enough for there to be a different cereal a
day (Monday - Friday).

The data should be normalized because features with larger magnitudes (like 
calories, potass, and sodium) will influence the model more than the other 
features.

If the data is not normalized you are forced to only use features with
similar scales or manually use your own weights to fix the scales.
<br>
